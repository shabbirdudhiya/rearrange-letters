...
var gameData = {page:'', questionNum:0, sequenceNum:0, categoryNum:0, category_arr:[], categoryThumb_arr:[], sequence_arr:[], targetArray:null, targetAnswerSequence:null, mode:'landscape', oldMode:'landscape', build:false, dragging:false, isUserScore:false, isUserCorrect:false, readyForNext:false, readyComplete:false, answered:false, settings:{}};
...
function animateTitle(){
	...
	TweenMax.to($('.titleWord').eq(titleData.shuffle[titleData.index]), titleSettings.tween, {scale:1.5, overwrite:true, ease:Sine.easeIn, onComplete:function(){
		TweenMax.to($('.titleWord').eq(titleData.shuffle[titleData.index]), titleSettings.tween, {scale:1, overwrite:true, ease:Sine.easeOut, onComplete:function(){
			titleData.index++;
			findNextTitle();
		}});
	}});		
...
function loadQuestion(){
	...
	if(descriptionAudio != ''){
		...
		fileFest.push({src:quesLandscape_arr[gameData.sequenceNum].description.audio, id:thisMode+'DescriptionAudio'});
	}
	...
	if(quesLandscape_arr[gameData.sequenceNum].descriptionType == 'image'){
		fileFest.push({src:quesLandscape_arr[gameData.sequenceNum].description.type, id:thisMode+'descriptionImage'})
	}
	...
	if(descriptionAudio != ''){
		...
		fileFest.push({src:quesPortrait_arr[gameData.sequenceNum].description.audio, id:thisMode+'DescriptionAudio'})
	}
...
function buildQuestion(){
	...
	TweenMax.to($('#questionWrapperHolder'), .5, {delay:.5, alpha:1, overwrite:true, onComplete:function(){
...
function animateWord(){
	...
	TweenMax.to($('#questionHolder'), 1, {overwrite:true, onComplete:function(){
		TweenMax.to( $('.dragFocus'), dragSettings.answerAnimateTween,{scale:dragSettings.answerAnimateScale, overwrite:true, onComplete:function(){
			TweenMax.to( $('.dragFocus'), dragSettings.answerAnimateTween,{scale:1, overwrite:true, onComplete:function(){
				questionData.reveal = false;
				updateGameStatus();
...
function displayGameStatus(con){
	...
	TweenMax.to($('#gameStatus .gameDisplay'), .3, {scale:1.5, overwrite:true, onComplete:function(){
		TweenMax.to($('#gameStatus .gameDisplay'), .3, {scale:1, overwrite:true, onComplete:function(){
			TweenMax.to($('#gameStatus .gameDisplay'), 2, {scale:1, overwrite:true, onComplete:function(){
				$('#gameStatus .gameDisplay').hide();
				updateGameStatus();
			}});
		}});
...
function playAudioLoop(con){
	...
	if(con == 'description'){
		...
		if(gameData.targetAudio[audioData.audioNum].type == 'description'){
			...
			TweenMax.to(audioData, audioSettings.description/1000, {overwrite:true, onComplete:function(){
		...
		TweenMax.to(audioData, audioSettings.answer/1000, {overwrite:true, onComplete:function(){
			playAudio(gameData.targetAudio[audioData.audioNum].id);
		}});
	}
}
	...
	$.ajax({
       ...
       success: function (result) {
			...
			$(result).find('thumb').each(function(catIndex, catElement){
				...
				gameData.categoryThumb_arr.push({src:$(catElement).text(), name:$(catElement).attr('name'), parent:parent});
			});
